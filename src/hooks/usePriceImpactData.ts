...
export function usePriceImpactData(blocks: MinimalBlock[]) {
  ...
  const fullData = useMemo(() => {
    ...
    const dailyStats = new Map<
      string,
      { rewardAlgo: number; blocks: number }
    >();
  ...
  const scatterData = useMemo(() => {
    return fullData.filter(
      (d) => d.rewardAlgo > 0 && d.valueAtReceipt !== null,
    );
  }, [fullData]);
  ...
  return {
    ...
    totalValueAtReceipt:
      fullData.length > 0
        ? fullData[fullData.length - 1].cumulativeValueAtReceipt
        : 0,
    totalValueAtToday:
      fullData.length > 0
        ? fullData[fullData.length - 1].cumulativeValueAtToday
        : 0,
  };
}
