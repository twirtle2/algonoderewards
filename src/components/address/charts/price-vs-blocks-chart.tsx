    ...
    for (let i = 0; i < result.length; i++) {
      if (result[i].priceAtReceipt === null) {
        ...
        while (
          ...
          result[nextIndex].priceAtReceipt === null
        ) {
          nextIndex++;
        }
        ...
        if (nextIndex < result.length && i > 0) {
          ...
          result[i] = {
            ...result[i],
            priceAtReceipt: prevPrice + step * (i - (i - 1)),
          };
        } else if (i > 0) {
          result[i] = {
            ...result[i],
            priceAtReceipt: result[i - 1].priceAtReceipt!,
          };
        } else if (nextIndex < result.length) {
          result[i] = {
            ...result[i],
            priceAtReceipt: result[nextIndex].priceAtReceipt!,
          };
        }
      }
    }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="mb-4 flex items-center justify-between">
        ...
        >
          <ToggleGroupItem value="blocks" className="h-6 px-2 text-[10px]">
            Blocks
          </ToggleGroupItem>
          <ToggleGroupItem value="reward" className="h-6 px-2 text-[10px]">
            ALGO
          </ToggleGroupItem>
        </ToggleGroup>
      </div>
