        ...
        <div className="flex flex-col h-full">
            <div className="space-y-1">
                ...
                <p className="text-[11px] leading-tight text-gray-500 dark:text-gray-400">
                    Reward size reflects Algorand's declining block reward schedule over time.
                    Colour indicates USD value at time of receipt.
                </p>
            </div>
                    ...
                    <ScatterChart margin={{ top: 10, right: 20, left: 0, bottom: 20 }}>
                        ...
                        <Tooltip
                            content={({ active, payload }) => {
                                if (active && payload && payload.length) {
                                    const d = payload[0].payload as PriceImpactDataPoint & { timestamp: number };
                                    return (
                                        <div className="rounded-md border border-gray-200 bg-white p-2 shadow-md dark:border-gray-700 dark:bg-gray-800">
                                            ...
                                            <div className="space-y-1 text-xs">
                                                <p className="flex justify-between gap-4">
                                                    <span className="text-gray-500 dark:text-gray-400 font-medium">Rewards:</span>
                                                    <span className="font-bold text-gray-900 dark:text-gray-100">
                                                        {hideBalance ? "*****" : `${d.rewardAlgo.toFixed(4)} ALGO`}
                                                    </span>
                                                </p>
                                                <p className="flex justify-between gap-4">
                                                    <span className="text-gray-500 dark:text-gray-400 font-medium">Value at Receipt:</span>
                                                    <span className="font-bold text-amber-500">
                                                        {hideBalance ? "*****" : formatCurrency(d.valueAtReceipt || 0)}
                                                    </span>
                                                </p>
                                                <p className="flex justify-between gap-4 border-t border-gray-100 pt-1 mt-1 dark:border-gray-700">
                                                    <span className="text-gray-500 dark:text-gray-400 font-medium">ALGO Price then:</span>
                        ...
                        <Scatter name="Rewards" data={chartData} shape="circle">
                            {chartData.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={getColor(entry.valueAtReceipt || 0)} fillOpacity={0.65} />
                            ))}
                        </Scatter>
                    </ScatterChart>
                </ResponsiveContainer>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-between gap-4 px-2 pb-4">
                <div className="flex flex-col gap-1.5">
                    <span className="text-[10px] uppercase tracking-wider text-gray-400 font-bold">USD Value at Receipt</span>
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-gray-500 dark:text-gray-400">Low</span>
                        <div
                            className="h-2 w-32 rounded-full"
                            style={{ background: 'linear-gradient(to right, #312e81, #7c3aed, #db2777, #ea580c)' }}
                        />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-500 dark:text-gray-400">ALGO Amount</span>
                    </div>
                </div>
            </div>
        </div>
