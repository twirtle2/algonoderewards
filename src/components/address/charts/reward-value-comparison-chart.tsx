...
const RewardValueComparisonChart = React.memo(
  function RewardValueComparisonChart({ data, hideBalance }: Props) {
    ...
    const isDepreciated =
      data.length > 0 &&
      data[data.length - 1].cumulativeValueAtToday <
        data[data.length - 1].cumulativeValueAtReceipt;
            ...
            >
              <defs>
                <linearGradient id="diffGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop
                    offset="5%"
                    stopColor={isDepreciated ? "#ef4444" : "#10b981"}
                    stopOpacity={0.3}
                  />
                  <stop
                    offset="95%"
