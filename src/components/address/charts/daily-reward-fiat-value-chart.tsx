...
const DailyRewardFiatValueChart = React.memo(
  function DailyRewardFiatValueChart({ data, hideBalance }: Props) {
            ...
            >
              ...
              <YAxis
                ...
                label={{
                  ...
                  style: { fontSize: "10px", fill: lineSeriesColor },
                }}
              />
              <Tooltip
                contentStyle={{
                ...
                formatter={(
                  ...
                  const algoVal =
                    numValue === 0
                      ? "No reward"
                      : `${numValue.toFixed(4)} ALGO`;
                  return [hideBalance ? "*****" : algoVal, name];
                }}
                // Add price to tooltip
                itemSorter={(item) =>
                  item.name === "Daily ALGO Reward" ? 1 : 0
                }
                content={({ active, payload, label }) => {
                  if (
                    ...
                    const dataPoint = payload[0]
                      .payload as PriceImpactDataPoint;
                    return (
                      <div className="rounded-md border border-gray-200 bg-white p-2 shadow-md dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <div className="space-y-1">
                          <p className="text-xs text-[#60a8fb]">
                            ...
                            {hideBalance
                              ...
                              : dataPoint.rewardAlgo === 0
                                ? "No reward"
                                : `${dataPoint.rewardAlgo.toFixed(4)}`}
                          </p>
                          ...
                          <p className="text-xs text-green-500">
                            ...
                            {dataPoint.priceAtReceipt !== null
                              ? `$${dataPoint.priceAtReceipt.toFixed(4)}`
                              : "Unknown"}
                          </p>
                        </div>
                      </div>
